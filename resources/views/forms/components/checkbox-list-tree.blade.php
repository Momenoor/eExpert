...
<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    ...
    >
        ...
        <x-filament::grid
            ...
            "
        >
            @forelse ($getOptions() as $value => $parent)
                @if(is_array($parent))
                    @php
                        $label = $parent['label'];
                ...
                >
                    ...
                    @forelse($children as $subValue => $subLabel)
                        <div
                            wire:key="{{ $this->getId() }}.{{ $statePath }}.{{ $field::class }}.options.{{ $subValue }}"
                            @if ($isSearchable)
                                x-show="
                            $el
                                .querySelector('.fi-fo-checkbox-list-option-label')
                        ...
                        >
                            ...
                            >
                                ...
                                \Filament\Support\prepare_inherited_attributes($getExtraInputAttributeBag())
                                    ->merge([
                                        'disabled' => $isDisabled || $isOptionDisabled($subValue, $subLabel),
                                        'value' => $subValue,
                                        'wire:loading.attr' => 'disabled',
                                        $applyStateBindingModifiers('wire:model') => $statePath,
                                        'x-on:change' => $isBulkToggleable ? 'checkIfAllCheckboxesAreChecked()' : null,
                            ...
                            >
                                ...
                                @endif
                                    ...
                                    @if ($hasDescription($subValue))
                                        <p
                                            class="fi-fo-checkbox-list-option-description text-gray-500 dark:text-gray-400"
                                        >
                                            {{ $getDescription($subValue) }}
                                        </p>
                                    @endif
                                </div>
                            </label>
