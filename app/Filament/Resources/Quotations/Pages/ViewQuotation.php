...
class ViewQuotation extends ViewRecord
{
     ...
     * hands off to `ContractResource`'s own view page rather than staying on
     * the quotation — the quotation itself doesn't change.
     */
    private function convertToContractAction(): Action
    {
        return Action::make('convert_to_contract')
            ->label(__('Convert to Contract'))
            ...
            ->action(function () {
                try {
                    $contract = app(ContractService::class)->createFromQuotation($this->quotation());
                ...
                Notification::make()->success()->title(__('Contract created.'))->send();

                return redirect(ContractResource::getUrl('view', ['record' => $contract]));
            });
    }
