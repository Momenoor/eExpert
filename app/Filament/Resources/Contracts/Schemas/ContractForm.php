...
/**
 * Raw-input contract creation — the other path, converting an accepted
 * `Quotation`, goes through `ContractService::createFromQuotation()` from
                    ...
                    ->schema([
                        Repeater::make('tenants')
                            ...
                            ->schema([
                                ...
                                Select::make('role')
                                    ->label(__('Role'))
                                    ->options(ContractPartyRole::class)
                                    ->default(ContractPartyRole::PRIMARY_TENANT->value)
                                    ->required(),
                            ])
                            ->columns(2)
                            ->minItems(1)
                        ...
                        Select::make('units')
                            ...
                            ->options(fn (): array => Unit::query()
                                ...
                                ->mapWithKeys(fn (Unit $unit): array => [
                                    $unit->id => "{$unit->building?->name} — {$unit->unit_number}",
                                ])->all())
                            ->multiple()
                            ->searchable()
                            ->required()
