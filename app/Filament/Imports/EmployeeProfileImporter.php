...
class EmployeeProfileImporter extends Importer
{
    ...
    {
        $columns = [
            ImportColumn::make('party_name')
                ->label(__('Employee Name'))
                ->requiredMapping()
                ->example('محمد أحمد الظاهري')
    ...
    {
        $name = trim((string) ($this->data['party_name'] ?? ''));

        $party = Party::withRole('employee')
            ->whereRaw('LOWER(name) = ?', [mb_strtolower($name)])
            ->first();

        if (! $party) {
            throw new RowImportFailedException(__(
                'No party named ":name" holding the Employee role was found. Add them as a party first.',
                ['name' => $name]
            ));
        }
